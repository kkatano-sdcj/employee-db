...
- **表示名・呼称の統一**: すべての画面（登録・編集・一覧・詳細）で「従業員番号」「社員番号」「従業員ID」を「社員コード」に、「時給」を「時給単価」に統一する（`docs/requirements/requirement-changes01.md` および `docs/requirements/requirement-changes02.md` 準拠）。データベースのカラム名やエンティティ名は後方互換性のため変更しないが、UI表示は統一する。
- **データベース入力項目**: CSVファイル（`【見本】パート入社登録情報_2_utf8.csv`）に含まれる全40項目をデータベースへの入力項目として定義し、入社時登録必須項目を明確化する（`docs/requirements/requirement-changes01.md` 準拠）。
- **表示データ項目の制限**: パート管理表（BPO）CSVファイルのデータ項目一覧に加えて、入社時登録必須項目のみを画面上に表示する。それ以外の項目は画面上に表示しない（`docs/requirements/requirement-changes02.md` 準拠）。各タブ（雇用情報、勤務情報、給与・手当、書類）の表示項目は `specs/008-comprehensive-spec/spec.md` の「従業員管理ページの表示データ項目」セクションに準拠する。
- **命名規則**: 従業員番号は「部門コード + 4桁の番号」（例：BPS0001）、契約番号は「従業員番号-CON + 4桁の番号」（例：BPS0001-CON00001）の形式で自動生成する（FR-096, FR-097準拠）。
- **承認番号管理**: 契約作成・更新時に承認番号を入力し、`employment_history` テーブルに保存する。契約履歴タブで契約番号の下に承認番号を表示する（FR-094, FR-095準拠）。
- **ページ遷移制御**: 契約更新・新規契約作成のページをそれぞれ独立して設定し、操作メニューから開く場合は従業員管理ページに遷移しない（FR-084, FR-098準拠）。
- **画面表示制御**: 従業員管理ページから開いた場合は基本情報のみ編集可能（勤務条件・雇用契約ウィジェット非表示）、契約管理ページから開いた場合は基本情報を固定表示し勤務条件・雇用契約のみ編集可能（FR-082, FR-083準拠）。
...
## コンテキストと方向性
- 現在の `apps/nextjs/src/app/contracts/page.tsx` では列が「従業員 / 契約タイプ / 期間 / 時給単価 / 状態 / アラート」のみで、契約番号・操作列・チェックボックス等が不足している。
...
## 目的 / 全体像
統括人事管理者が契約管理ページまたは従業員詳細ページから「契約書PDF」「誓約書PDF」をワンクリックで出力でき、PDF内には契約番号・従業員基本情報・雇用期間・勤務条件・賃金・交通費などがspec 008で指定された赤字領域に差し込まれます。契約書PDF生成時には雇用契約の枝番（contracts.id）がそのまま出力され、誓約書は社員コードのみを埋め込んだフォーマットとなります。
...
## コンテキストと方向性
- テンプレート: `docs/format/パート雇入通知書見本ver_A2025.09.docx`, `docs/format/誓約書.pdf` を参考に、必要フィールド（従業員名、契約期間、勤務条件、賃金、交通費、社員コードなど）をspecに従って差し込み。
...
1. 従業員マスター CRUD（`apps/nextjs/src/server/actions/create-employee.ts` / `apps/nextjs/src/server/queries/employees.ts`）  
   ...
   - **入社時登録必須項目**: 社員コード、フリガナ（氏名フリガナ）、氏名、性別、生年月日、フセン項目、入社日、郵便番号（住民票住所）、住所1/2（住民票住所）、住所1/2フリガナ（住民票住所）、電話番号1（個人用）、メールアドレス1（個人用）、健康保険加入区分、厚生年金加入区分、基礎年金番号、厚生年金基金加入区分、雇用保険区分、通勤費区分、支払順位、取扱区分、支払区分、口座名、振込期間コード、機関名、支店名、預金種目、口座番号、時給単価を必須項目として実装（`docs/requirements/requirement-changes01.md` 準拠）。  
   - **データベース入力項目**: CSVファイル（`【見本】パート入社登録情報_2_utf8.csv`）に含まれる全40項目を入力可能にする。現行の仕様で定義されているデータ項目は全て残し、同じ内容で表記ゆれがあるものは統一する（`docs/requirements/requirement-changes01.md` 準拠）。  
   - **表示名統一**: UI表示では「従業員番号」「社員番号」「従業員ID」を「社員コード」に、「時給」を「時給単価」に統一する（`docs/requirements/requirement-changes01.md` および `docs/requirements/requirement-changes02.md` 準拠）。データベースのカラム名は変更しない（後方互換性のため）。  
   - **従業員番号の命名規則**: 従業員番号は「部門コード + 4桁の番号」の形式で自動生成する。部門選択で部門コードが自動的に振られ、同じ部門コード内の連番とする（FR-096準拠）。例：BPS0001, BPS0002, ONS0001, CC0001, PS0001。
   - **画面表示制御**: 従業員管理ページから「従業員情報を編集」画面または「新規従業員登録」画面を開いた場合、基本情報の編集のみができ、勤務条件と雇用契約のウィジェットは表示しない（FR-082, User Story 6-1準拠）。
   - `bulkUpdateStatus` を従業員一覧のチェックボックス操作から呼び出し、UI-002（一覧からのステータス一括変更）を満たす。  
   - `search` は氏名/社員コード/勤務場所/雇用区分/ステータスを filter し、最大50件/ページ、p95 2秒以内を計測。
...
3. 従業員詳細タブ実装（`apps/nextjs/src/app/(dashboard)/employees/[id]/page.tsx`）  
   ...
   - **表示項目の制限**: パート管理表（BPO）CSVファイルのデータ項目と入社時登録必須項目のみを表示し、それ以外の項目は表示しない（`docs/requirements/requirement-changes02.md` 準拠）。各タブの表示項目は以下の通り：
     - **雇用情報タブ**: 社員コード、氏名、部門、契約番号、入社日、雇用期間、退社日
     - **勤務情報タブ**: 契約書有給、勤務時間、休憩時間、勤務日数/週、勤務場所、業務内容
     - **給与・手当タブ**: 時給単価、残業時給、最寄り駅、交通費（片道/往復）、控除申告書（甲乙）、雇用保険（加入/未加入）、雇用保険書提出、社会保険（加入/未加入）
     - **書類タブ**: 保険証授、雇用契約書他管理へ提出(日付)、本人へ返却、満了通知書発効、退職届提出、返却（保険証、セキュリティカード）
...
4. 従業員一覧 + フィルター（`apps/nextjs/src/app/(dashboard)/employees/page.tsx`）  
   - サーバーコンポーネントで `fetchEmployees` を呼び、検索フォーム（氏名/社員コード/部門/雇用区分/ステータス/勤務場所）を実装。  
...
**目的**: 契約期間管理・「要更新」バッジ・契約操作メニュー・契約入力UI・契約履歴自動追記・契約更新アラートを `specs/008` User Story 3/4/5/6/7/8/11/14 に沿って実装し、SC-002/SC-005/SC-007/SC-008 を満たす。雇用満了予定日（`employment_expiry_scheduled_date`）を唯一の源泉とし、PDF出力時にアラートを自動解除する。契約番号の命名規則を実装し、承認番号を管理する。
**仕様リンク**: `specs/008-comprehensive-spec/spec.md` 「User Story 3〜8,11,14」「Key Entities: Contract/Contract Alert/Contract Change History」「成功基準 SC-002〜SC-008」「UI-011〜UI-019」「FR-082〜FR-098」  
...
8. 契約番号の命名規則と自動生成（`apps/nextjs/src/server/actions/upsert-contract.ts`）
   - 契約番号を「従業員番号-CON + 4桁の番号」の形式で自動生成する（FR-097準拠）
   - 同じ従業員の持つ番号の連番とする。例：BPS0001-CON00001, BPS0001-CON00002, ONS0001-CON00001
   - 契約作成時に自動的に採番し、契約更新時には既存の契約番号を表示し、必要に応じて変更できるようにする（FR-086準拠）
9. 承認番号の管理（`apps/nextjs/src/server/actions/upsert-contract.ts` / `apps/nextjs/src/server/queries/employment-history.ts`）
   - 契約更新ページと新規契約作成ページに承認番号の入力欄を設ける（FR-087準拠）
   - 契約作成・更新時に `employment_history` テーブルに承認番号を保存する（FR-095準拠）
   - 契約履歴タブで契約番号の下に承認番号を表示する（FR-094準拠）
10. ページ遷移制御とページ独立設定（`apps/nextjs/src/app/(dashboard)/contracts/*`）
   - 「契約更新」と「新規契約作成」のページをそれぞれ独立して設定する（FR-084準拠）
   - 操作メニューから契約更新または新規契約作成を開く場合、従業員管理ページには遷移せず直接該当画面を表示する（FR-098準拠）
11. 画面表示制御とUI実装（`apps/nextjs/src/app/(dashboard)/contracts/[id]/edit/page.tsx` / `apps/nextjs/src/app/(dashboard)/contracts/new/page.tsx`）
   - 契約更新・新規契約作成ページでは、従業員の基本情報をデータベースから取得してウィジェットの外に固定表示する（FR-083準拠）
   - 勤務条件と雇用契約のみを入力欄とし、新規契約作成では書類・提出状況と社会保険・給与情報も入力可能とする（FR-083準拠）
   - 契約番号入力欄を先頭に配置する（FR-085準拠）
- **DoD**
...
- 契約更新/削除/再作成時に employment_history と書類履歴へ自動追記され、承認番号も保存される（User Story 7, 14, FR-094, FR-095）
- 契約番号が「従業員番号-CON + 4桁の番号」の形式で自動生成される（FR-097）
- 契約履歴タブで契約番号の下に承認番号が表示される（FR-094, User Story 7）
- 契約更新・新規契約作成のページがそれぞれ独立して設定され、操作メニューから開く場合は従業員管理ページに遷移しない（FR-084, FR-098）
- 契約更新・新規契約作成ページで基本情報が固定表示され、勤務条件と雇用契約のみが入力欄となる（FR-083）
- 契約番号入力欄が先頭に配置される（FR-085）
- 承認番号入力欄が契約更新・新規契約作成ページに表示される（FR-087）
**Risk**
...
**目的**: 時給単価・勤務条件・社保フラグ・各種手当を含む給与計算用データ出力  
**技術実装**: Prisma + CSV生成（papaparse）
**Steps**
1. 給与計算用データの抽出仕様定義
   - 従業員情報: employees テーブル（employeeNumber, name）
   - 時給単価情報: contracts.hourlyWage（`table.md` の contracts テーブル定義を参照）
...
- 2025-01-28: 要件定義変更をPLANS.mdに反映（`docs/requirements/requirement-changes01.md` 準拠）
  - Global Goal: 表示名・呼称の統一（従業員番号→社員コード、時給→時給単価）とデータベース入力項目の追加を明記
  - Phase 2: 入社時登録必須項目（30項目）とデータベース入力項目（40項目）の実装要件を追加
  - Phase 2: UI表示での「社員コード」「時給単価」への統一を明記（データベースカラム名は変更しない）
  - Phase 4: 契約管理画面の列表示で「時給」を「時給単価」に統一
  - Phase 5: 誓約書PDF出力で「社員番号」を「社員コード」に統一
  - Phase 6: CSV抽出項目で「従業員番号」を「社員コード」に統一
  - Phase 7: 給与支払データ抽出で「時給」を「時給単価」に統一
  - 注意事項: 現行の仕様で定義されているデータ項目は全て残し、表記ゆれがあるものは統一する。データベースのカラム名やエンティティ名の変更は後方互換性を考慮して段階的に実施する。
- 2025-01-28 (requirement-changes02): 表示データ項目の制限要件をPLANS.mdに反映（`docs/requirements/requirement-changes02.md` 準拠）
  - Global Goal: 表示データ項目の制限要件を追加（パート管理表（BPO）CSVファイルのデータ項目と入社時登録必須項目のみを表示、それ以外は表示しない）
  - Phase 2: 従業員詳細タブ実装に表示項目の制限要件を追加
    - 各タブ（雇用情報、勤務情報、給与・手当、書類）の表示項目を明確化
    - 表示名の統一について `requirement-changes02.md` も参照するように更新
  - 仕様書（`specs/008-comprehensive-spec/spec.md`）の「従業員管理ページの表示データ項目」セクションに表示項目の制限要件を追加済み
  - 注意事項: データベーススキーマの変更は不要（UI表示のみの制限のため）。現行の仕様で定義されているデータ項目は全て残し、表示項目のみを制限する。
- 2025-01-28 (requirement-changes03): 画面表示とページ遷移の変更、命名規則、承認番号管理をPLANS.mdに反映（`docs/requirements/requirement-changes03.md` 準拠）
  - Global Goal: 従業員番号・契約番号の命名規則と承認番号管理を追加
  - Phase 2: 従業員番号の命名規則（部門コード + 4桁の番号）を実装（FR-096）
    - 従業員番号の自動生成ロジックを追加（例：BPS0001, ONS0001, CC0001, PS0001）
    - 部門選択で部門コードが自動的に振られ、同じ部門コード内の連番とする
  - Phase 2: 契約番号の命名規則（従業員番号-CON + 4桁の番号）を実装（FR-097）
    - 契約番号の自動生成ロジックを追加（例：BPS0001-CON00001, BPS0001-CON00002）
    - 同じ従業員の持つ番号の連番とする
  - Phase 2: 従業員管理ページから「従業員情報を編集」画面または「新規従業員登録」画面を開いた場合、勤務条件と雇用契約のウィジェットを表示しない（FR-082更新、User Story 6-1更新）
  - Phase 4: 契約更新・新規契約作成ページでは従業員の基本情報をデータベースから取得してウィジェットの外に固定表示し、勤務条件と雇用契約のみを入力欄とする（FR-083更新、User Story 6更新）
    - 新規契約作成では書類・提出状況と社会保険・給与情報も入力可能
  - Phase 4: 「契約更新」と「新規契約作成」のページをそれぞれ独立して設定（FR-084更新、User Story 5更新）
  - Phase 4: 契約番号入力欄を先頭に配置（FR-085）
  - Phase 4: 契約番号の入力・変更機能を実装（FR-086）
  - Phase 4: 承認番号入力欄を契約更新・新規契約作成ページに追加（FR-087）
  - Phase 4: 契約履歴タブの契約番号の下に承認番号を表示（FR-094、User Story 7更新）
  - Phase 4: employment_historyテーブルに承認番号（approval_number）フィールドを追加（FR-095）
