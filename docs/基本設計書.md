...
4. **ページ実装（Plan3反映）**  
   ...
   - `app/(dashboard)/employees/[id]/page.tsx`: タブUI (基本情報/勤務条件/契約/履歴) + `WorkConditionTimeline`.  
...
5. フロントエンド実装（`apps/nextjs/src/app/(dashboard)/employees/`）
   ...
   - 従業員詳細画面: タブ UI（基本情報/勤務条件/雇用契約/履歴）
...
5. 列レベル制御
   - `myNumber` (個人番号), `hourlyWage` (給与情報), `overtimeHourlyWage`（残業時給）は統括/管理者のみ取得可能
