...
**Created**: 2025-11-19  
**Last Updated**: 2025-01-28 (画面表示とページ遷移の変更、契約番号・従業員番号の命名規則、承認番号管理の追加)  
**Status**: Draft  
...
9. **Given** 統括人事管理者が3点メニューから「契約更新」を選択する, **When** メニュー項目をクリックする, **Then** 契約更新画面が表示され、従業員管理ページには遷移しない
10. **Given** 統括人事管理者が3点メニューから「新規契約作成」を選択する, **When** メニュー項目をクリックする, **Then** 新規契約作成画面が表示され、従業員管理ページには遷移しない
11. **Given** 統括人事管理者が契約更新または新規契約作成画面を表示する, **When** 画面を確認する, **Then** 契約更新と新規契約作成のページがそれぞれ独立して設定されている
12. **Given** 統括人事管理者が3点メニューから「契約削除」を選択する, **When** メニュー項目をクリックし、削除確認ダイアログで確認する, **Then** 契約が削除される

---

### User Story 6 - 契約作成・更新画面での情報入力 (Priority: P1)

統括人事管理者が契約の新規作成または更新画面で、work_conditionsテーブルとcontractsテーブルの情報を入力・表示できる。契約更新または新規契約作成画面では、従業員の基本情報はデータベースから取得してウィジェットの外に固定表示し、勤務条件と雇用契約のみを入力欄とする。新規契約作成ページでは、書類・提出状況と社会保険・給与情報も入力可能とする。また、契約番号と承認番号の入力欄を設ける。

**優先度の理由**: 契約作成・更新時に必要な情報を効率的に入力できることが重要です。従業員の基本情報を固定表示することで、誤操作を防ぎ、契約情報の入力に集中できます。勤務条件と雇用契約の入力を容易にすることで、作業効率が向上し、データの整合性も保たれます。

**独立テスト**: 統括人事管理者が新規契約作成画面を表示し、従業員の基本情報がデータベースから取得されて固定表示され、勤務条件（勤務時間、休憩時間、勤務場所、交通費など）と雇用契約（契約種別、契約期間、時給など）を入力できることを確認します。契約更新画面でも同様に、基本情報が固定表示され、勤務条件と雇用契約を編集できることを検証できます。また、契約番号と承認番号の入力欄が存在することを確認します。
...
5. **Given** 統括人事管理者が契約更新または新規契約作成画面を表示している, **When** 従業員の基本情報を確認する, **Then** 基本情報はデータベースから取得されてウィジェットの外に固定表示され、編集できない
6. **Given** 統括人事管理者が契約更新または新規契約作成画面を表示している, **When** 勤務条件と雇用契約の入力欄を確認する, **Then** これらの入力欄が編集可能である
7. **Given** 統括人事管理者が新規契約作成画面を表示している, **When** 書類・提出状況と社会保険・給与情報の入力欄を確認する, **Then** これらの入力欄も編集可能である
8. **Given** 統括人事管理者が契約更新または新規契約作成画面を表示している, **When** 画面の先頭を確認する, **Then** 契約番号と承認番号の入力欄が表示される
9. **Given** 統括人事管理者が契約更新または新規契約作成画面で契約番号と承認番号を入力する, **When** 契約番号と承認番号を入力して保存する, **Then** 契約番号と承認番号が正しく保存される

---

### User Story 6-1 - 従業員情報編集画面での編集権限制御 (Priority: P1)

統括人事管理者が従業員管理ページから「従業員情報を編集」画面または「新規従業員登録」画面を開いた場合、基本情報の編集のみができ、勤務条件と雇用契約のウィジェットは表示しない。

**優先度の理由**: 従業員情報の編集と契約情報の編集を分離することで、誤操作を防ぎ、データの整合性を保つことができます。基本情報のみを編集できるようにし、契約関連の情報は契約管理ページから管理することで、業務フローが明確になり、作業効率が向上します。

**独立テスト**: 統括人事管理者が従業員管理ページから「従業員情報を編集」画面または「新規従業員登録」画面を開き、基本情報のみが編集可能で、勤務条件と雇用契約のウィジェットが表示されていないことを確認します。

**受け入れシナリオ**:

1. **Given** 統括人事管理者が従業員管理ページから「従業員情報を編集」画面または「新規従業員登録」画面を開いている, **When** 基本情報ウィジェットを確認する, **Then** 基本情報が編集可能である
2. **Given** 統括人事管理者が従業員管理ページから「従業員情報を編集」画面または「新規従業員登録」画面を開いている, **When** 画面全体を確認する, **Then** 勤務条件と雇用契約のウィジェットは表示されていない
3. **Given** 統括人事管理者が従業員管理ページから「従業員情報を編集」画面で基本情報を変更する, **When** 変更内容を保存する, **Then** 基本情報が正しく更新される
4. **Given** 統括人事管理者が契約情報を編集したい, **When** 従業員管理ページから「従業員情報を編集」画面を開く, **Then** 勤務条件と雇用契約の編集はできず、契約管理ページから行う必要がある

---

### User Story 7 - 契約履歴への自動追記 (Priority: P1)

新しい契約が作成されたり、契約が更新された場合、employment_historyテーブルに登録された契約番号ごとに、勤務条件、雇用契約、書類・提出状況、承認番号が自動的に保存される。勤務条件、雇用契約、書類・提出状況は1つの契約番号で管理され、その契約番号と承認番号に関連するすべての情報がemployment_historyテーブルに記録される。

**優先度の理由**: 契約の変更履歴を正確に記録することは、監査や法的要件の遵守のために重要です。契約の作成・更新時に自動的に履歴が記録されることで、手動での記録漏れを防ぎます。契約番号と承認番号ごとに勤務条件、雇用契約、書類・提出状況を統合して保存することで、契約の完全な履歴を追跡できます。

**独立テスト**: 統括人事管理者が新規契約を作成し、契約番号、承認番号とともに勤務条件、雇用契約、書類・提出状況を入力して保存した後、employment_historyテーブルに新しいレコードが作成され、契約番号、承認番号に関連するすべての情報（勤務条件、雇用契約、書類・提出状況）が記録されることを確認します。契約を更新した場合も、同様に履歴が追記されることを検証できます。

**受け入れシナリオ**:

1. **Given** 統括人事管理者が新規契約を作成する, **When** 契約番号、承認番号とともに勤務条件、雇用契約、書類・提出状況の情報を入力して保存する, **Then** employment_historyテーブルに新しいレコードが作成され、契約番号、承認番号とその契約番号に関連する勤務条件、雇用契約、書類・提出状況が記録される
2. **Given** 統括人事管理者が既存の契約を更新する, **When** 契約番号、承認番号に関連する勤務条件、雇用契約、書類・提出状況の情報を変更して保存する, **Then** employment_historyテーブルに新しいレコードが作成され、更新後の契約番号、承認番号とその契約番号に関連する勤務条件、雇用契約、書類・提出状況が記録される
3. **Given** 統括人事管理者が従業員の契約履歴を表示する, **When** 契約履歴タブを確認する, **Then** 新規作成・更新された契約の情報（契約番号、承認番号、勤務条件、雇用契約、書類・提出状況）が時系列で表示され、契約番号の下に承認番号が表示される
4. **Given** 統括人事管理者が複数の契約番号を持つ従業員の履歴を表示する, **When** 契約履歴タブを確認する, **Then** 各契約番号ごとに、その契約番号と承認番号に関連する勤務条件、雇用契約、書類・提出状況が正しく表示される
...
### Session 2025-01-28 (Update 2)

- 画面表示とページ遷移の変更: 契約更新と新規契約作成のページをそれぞれ独立して設定し、操作メニューから開く場合は従業員管理ページに遷移しない（FR-084、User Story 5更新として要件化）
- 契約管理ページの入力項目制限: 契約更新・新規契約作成ページでは従業員の基本情報をデータベースから取得してウィジェットの外に固定表示し、勤務条件と雇用契約のみを入力欄とする。新規契約作成では書類・提出状況と社会保険・給与情報も入力可能（FR-083更新、User Story 6更新として要件化）
- 従業員管理ページの表示制限: 新規従業員登録ページと従業員情報編集ページでは、勤務条件と雇用契約のウィジェットを表示しない（FR-082更新、User Story 6-1更新として要件化）
- 承認番号管理の追加: 契約履歴タブの契約番号の下に承認番号を表示し、employment_historyテーブルに承認番号フィールドを追加（FR-094、FR-095、User Story 7更新、Employment Historyエンティティ更新として要件化）
- 従業員番号の命名規則: 従業員番号を部門コード + 4桁の番号の形式で生成（例：BPS0001、ONS0001、CC0001、PS0001）（FR-096として要件化）
- 契約番号の命名規則: 契約番号を「従業員番号-CON + 4桁の番号」の形式で生成（例：BPS0001-CON00001、BPS0001-CON00002）（FR-097として要件化）
- ページ遷移制御: 操作メニューから契約更新または新規契約作成を開く場合、従業員管理ページに遷移せず直接該当画面を表示（FR-098として要件化）
...
- **FR-058**: システムは3点メニューアイコンをクリックした際、ポップアップメニューを表示し、「契約書プレビュー」「誓約書プレビュー」「契約更新」「新規契約作成」「契約削除」の5つのメニュー項目を表示すること。
- **FR-059**: システムは「契約書プレビュー」または「誓約書プレビュー」メニューを選択した際、ポップアップで書類の内容を表示すること。
...
- **FR-082**: システムは従業員管理ページから「従業員情報を編集」画面を開いた場合、基本情報のウィジェットのみが編集可能とし、勤務条件と雇用契約のウィジェットは表示しないこと。
- **FR-083**: システムは契約管理ページから「契約更新」または「新規契約作成」画面を開いた場合、従業員の基本情報はデータベースから取得してウィジェットの外に固定表示し、勤務条件と雇用契約のみを入力欄とすること。また、書類・提出状況と社会保険・給与情報も新規契約作成時には入力可能とすること。
- **FR-084**: システムは「契約更新」と「新規契約作成」のページをそれぞれ独立して設定すること。
- **FR-085**: システムは契約更新ページと新規契約作成ページで、契約番号を入力できる記入欄を先頭に配置すること。
- **FR-086**: システムは契約番号の入力欄で、新規契約作成時には新規の契約番号を入力でき、契約更新時には既存の契約番号を表示し、必要に応じて変更できること。
- **FR-087**: システムは契約更新ページと新規契約作成ページに承認番号の入力欄を設けること。
...
- **FR-094**: システムは従業員詳細ページの契約履歴タブで、契約番号の下に承認番号を表示すること。
- **FR-095**: システムはemployment_historyテーブルに承認番号（approval_number）フィールドを追加し、承認番号を登録・保存できること。
- **FR-096**: システムは従業員番号を部門コード + 4桁の番号の形式で生成すること。部門コードは部門選択で自動的に振られ、番号は同じ部門コード内の連番とすること。
  - 部門名と部門コード：
    - BPS課: BPS
    - オンサイト課: ONS
    - CC課: CC
    - PS課: PS
  - 例：BPS0001, BPS0002, ONS0001, CC0001, PS0001
- **FR-097**: システムは契約番号を「従業員番号-CON + 4桁の番号」の形式で生成すること。契約識別番号の数字は、同じ従業員の持つ番号の連番とすること。
  - 例：
    - BPS0001-CON00001
    - BPS0001-CON00002
    - BPS0002-CON00001
    ...
    - PS0001-CON00001
    - PS0001-CON00002
- **FR-098**: システムは従業員の情報を「操作」のポップアップメニューから開く場合、従業員管理ページには遷移せず、直接契約更新または新規契約作成の画面を表示すること。
...
- **雇用履歴（Employment History）**: 雇用・人事履歴を表し、期間、部門（`部門情報.md`に記載されている4つの項目（BPS課、オンサイト課、CC課、PS課）のいずれか）、有給休暇、給与参照、勤務場所履歴、勤務日数タイプ（週/月/シフト）、契約番号（contracts.idへの参照）、承認番号（approval_number）、有効日（effective_date）、イベント種別（event_type）、契約内容の記録を含む。登録された契約番号ごとに、その契約番号に関連する勤務条件、雇用契約、書類・提出状況を保存する。承認番号は契約履歴タブに表示され、employment_historyテーブルに保存される。注記：役職と等級はパート従業員には不要なため含まれない。最終更新日時と更新者情報を含む。
